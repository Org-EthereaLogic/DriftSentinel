...
EXPECTED_COMMANDS = [
    "prime", "start", "plan", "implement", "review", "verify", "test",
    "audit", "feature", "bug", "patch", "chore", "classify_issue",
    "generate_branch_name", "commit", "pull-request", "git", "doc-maintain",
    "document", "cleanup_workspace", "sync",
]


@pytest.mark.parametrize("cmd", EXPECTED_COMMANDS)
def test_command_exists(cmd: str) -> None:
    assert (ROOT / ".claude" / "commands" / f"{cmd}.md").is_file(), (
        f"Missing command: {cmd}.md"
    )
...
def test_hook_exists(hook: str) -> None:
    assert (ROOT / ".claude" / "hooks" / hook).is_file(), (
        f"Missing hook: {hook}"
    )
